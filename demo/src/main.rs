            ...
            Event::WindowEvent {
                ...
                match event {
                    ...
                    WindowEvent::KeyboardInput {
                        ...
                        if *state == ElementState::Pressed && *scancode == 57 {
                            let cam_view = renderer.camera.view;
                            let cam_proj = glam::Mat4::perspective_rh(
                                camera.projection.fovy,
                                camera.projection.aspect,
                                camera.projection.znear,
                                20.0,
                            );

                            let inv_cam = (cam_proj * cam_view).inverse();

                            scene.models.push(Box::new(shapes::SimpleMesh::new(
                                &renderer,
                                shapes::SimpleShape::Cube,
                                "Cube",
                                inv_cam,
                            ...
                            use glam::swizzles::*;
                            let light_dir = my_app.shadow_light_angle.normalize();
                            ...
                            let center_w = corners.iter().fold(glam::Vec3::ZERO, |acc, &v| acc + v)
                                / corners.len() as f32;

                            let mut radius = corners
                                .iter()
                                .fold(0.0_f32, |acc, &v| acc.max(v.distance(center_w)));

                            // Avoid shadow swimming
                            radius = (radius * 16.0).ceil() / 16.0; // Prevent small radius changes
                            let texel_size = radius * 2.0 / 1024.0; // Shadow texel size in light view space

                            let lv =
                                glam::Mat4::look_at_rh(glam::Vec3::ZERO, light_dir, glam::Vec3::Y);
                            let center = lv * center_w.extend(1.0);
                            let mut center = (center / texel_size).ceil() * texel_size;
                            center.z += radius;

                            let light_view = glam::Mat4::from_translation(-center.xyz()) * lv;
