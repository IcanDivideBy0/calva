...
[[group(2), binding(0)]] var<uniform> light_camera: LightCamera;
...
fn main([[builtin(vertex_index)]] vertex_index : u32) -> VertexOutput {
    ...
    // Get light dir in view space
    let light_view_inv = transpose(mat3x3<f32>(
        light_camera.view.x.xyz,
        light_camera.view.y.xyz,
        light_camera.view.z.xyz,
    ));
    ...
    let light_dir = view_rot * light_view_inv * vec3<f32>(0.0, 0.0, 1.0);

    return VertexOutput (clip, clip.xy, normalize(light_dir));
}
...
fn main(
    ...
    let frag_proj = light_camera.view_proj * frag_pos_world;
